...
function AdminScreen({ children, description, desktop, eyebrow, hideHeading = false, pathname, title, width }: ScreenProps & { desktop: boolean; pathname: string; width: number }) {
  ...
  return <SafeAreaView style={[styles.safe, dark && styles.adminDarkSafe]}>
    ...
    {!desktop ? <ScrollView contentContainerStyle={styles.adminMobileNav} horizontal showsHorizontalScrollIndicator={false} style={styles.adminMobileNavScroll}>{adminLinks.map((link) => <AdminNavLink key={link.href} link={link} pathname={pathname} collapsed={false} onOpen={open} />)}</ScrollView> : null}
...
const styles = StyleSheet.create({
  ...
  adminMobileNavScroll: { backgroundColor: colors.navy, flexGrow: 0, height: 56, maxHeight: 56 }, adminMobileNav: { alignItems: "center", backgroundColor: colors.navy, flexDirection: "row", gap: 4, height: 56, paddingHorizontal: 8 }, adminMain: { flex: 1, minWidth: 0 }, adminPage: { alignSelf: "center", maxWidth: 1440, minWidth: 0, paddingBottom: spacing.xl, paddingHorizontal: spacing.lg, width: "100%" }, adminMobilePage: { paddingHorizontal: spacing.sm }, adminBreadcrumb: { alignItems: "center", flexDirection: "row", gap: 4, minHeight: 42 }, breadcrumbMuted: { color: colors.muted, fontSize: 11, fontWeight: "700" }, breadcrumbCurrent: { color: colors.text, fontSize: 11, fontWeight: "900" },
