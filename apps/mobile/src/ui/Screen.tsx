...
function AdminScreen({ children, description, desktop, eyebrow, hideHeading = false, pathname, title }: ScreenProps & { desktop: boolean; pathname: string }) {
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={styles.adminTopbar}>
      ...
      {desktop ? <View style={styles.adminSearchWrap}><View style={[styles.adminSearch, searchFocused && styles.adminSearchFocused]}><Ionicons color={colors.muted} name="search-outline" size={18} /><TextInput accessibilityLabel="Search admin pages" autoCapitalize="none" onBlur={() => setTimeout(() => setSearchFocused(false), 120)} onChangeText={setSearchQuery} onFocus={() => setSearchFocused(true)} onSubmitEditing={submitSearch} placeholder="Search admin pages..." placeholderTextColor="#94a3b8" returnKeyType="go" style={styles.adminSearchInput} value={searchQuery} />{searchQuery ? <Pressable accessibilityLabel="Clear admin search" onPress={() => setSearchQuery("")}><Ionicons color={colors.muted} name="close-circle" size={18} /></Pressable> : null}</View>{searchFocused && normalizedSearch ? <View style={styles.adminSearchResults}>{searchResults.length ? searchResults.map((link) => <Pressable accessibilityRole="button" key={link.href} onPress={() => open(link.href)} style={styles.adminSearchResult}><View style={styles.adminSearchIcon}><Ionicons color={colors.primary} name={link.icon as never} size={17} /></View><View style={styles.adminSearchCopy}><Text style={styles.adminSearchLabel}>{link.label}</Text><Text style={styles.adminSearchPath}>{link.href}</Text></View><Ionicons color={colors.muted} name="arrow-forward" size={15} /></Pressable>) : <View style={styles.adminSearchEmpty}><Ionicons color={colors.muted} name="search-outline" size={18} /><Text style={styles.adminSearchEmptyText}>No admin page matches “{searchQuery.trim()}”</Text></View>}</View> : null}</View> : null}
