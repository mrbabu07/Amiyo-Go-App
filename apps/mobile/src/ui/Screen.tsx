...
const adminGroups: AdminGroup[] = [
  ...
  { label: "Orders", icon: "bag-handle-outline", links: [
    ...
    { label: "My Orders", href: "/admin/logistics?tab=work", icon: "clipboard-outline" },
    { label: "Ready Pickup", href: "/admin/logistics?tab=ready", icon: "cube-outline" },
    { label: "Dispatch Manifest", href: "/admin/logistics?tab=manifest", icon: "document-text-outline" },
    { label: "Parcel Assignment", href: "/admin/logistics?tab=parcels", icon: "git-branch-outline" },
    { label: "Pickup Staff", href: "/admin/logistics?tab=staff", icon: "people-outline" },
    { label: "COD Float", href: "/admin/logistics?tab=cod", icon: "wallet-outline" },
    { label: "Failed Delivery", href: "/admin/logistics?tab=failed", icon: "warning-outline" },
...
const adminStandaloneLinks: WorkspaceLink[] = [{ label: "Delivery Settings", href: "/admin/delivery-settings", icon: "car-outline" }];
const adminLinks = [...adminPrimaryLinks, ...adminGroups.flatMap((group) => group.links), ...adminStandaloneLinks];
...
export function Screen(props: ScreenProps) {
  ...
  if (workspace === "admin") return <AdminScreen {...props} desktop={width >= 1024} pathname={pathname} width={width} />;
  return <StandardScreen {...props} pathname={pathname} vendor={workspace === "vendor"} width={width} />;
}

function AdminScreen({ children, description, desktop, eyebrow, hideHeading = false, pathname, title, width }: ScreenProps & { desktop: boolean; pathname: string; width: number }) {
  ...
  const logout = async () => { if (firebaseAuth) await signOut(firebaseAuth); open("/login"); };

  return <SafeAreaView style={[styles.safe, dark && styles.adminDarkSafe]}>
    <View style={[styles.adminTopbar, !desktop && styles.adminMobileTopbar, dark && styles.adminDarkTopbar]}>
      <View style={styles.adminTopLeft}>
        <Pressable accessibilityLabel={desktop ? (collapsed ? "Expand admin sidebar" : "Collapse admin sidebar") : (mobileSidebarOpen ? "Close admin menu" : "Open admin menu")} onPress={() => desktop ? setCollapsed((value) => !value) : setMobileSidebarOpen((value) => !value)} style={styles.topIcon}><Ionicons color={topbarIcon} name={desktop ? (collapsed ? "menu-outline" : "menu") : (mobileSidebarOpen ? "close" : "menu")} size={22} /></Pressable>
      ...
      <View style={[styles.workspaceActions, !desktop && styles.adminMobileActions]}>
        ...
        {desktop ? <Pressable accessibilityLabel={`Account: ${accountName}, ${roleLabel(accountRole)}`} onPress={() => open("/account")} style={[styles.adminAccount, dark && styles.adminDarkAccount]}><View style={styles.avatar}><Text style={styles.avatarText}>{accountInitials}</Text></View><View><Text numberOfLines={1} style={[styles.accountName, dark && styles.adminDarkText]}>{accountName}</Text><Text style={[styles.accountRole, dark && styles.adminDarkMuted]}>{roleLabel(accountRole)}</Text></View></Pressable> : null}
        <Pressable accessibilityLabel="Sign out" onPress={logout} style={[styles.iconButton, styles.logoutButton, dark && styles.adminDarkButton]}><Ionicons color={colors.danger} name="log-out-outline" size={19} /></Pressable>
      </View>
    </View>
    {!desktop ? <AdminMobileSearch dark={dark} onOpen={open} /> : null}
    {!desktop && mobileSidebarOpen ? <View style={styles.adminDrawerLayer}>
      <Pressable accessibilityLabel="Close admin menu" onPress={() => setMobileSidebarOpen(false)} style={styles.adminDrawerBackdrop} />
      <View accessibilityLabel="Admin navigation" style={[styles.adminDrawer, { width: Math.min(width - 48, 288) }]}>
        <AdminSidebarNavigation collapsed={false} expandedGroups={expandedGroups} onOpen={open} pathname={pathname} setExpandedGroups={setExpandedGroups} />
      </View>
    </View> : null}
    <View style={styles.adminBody}>
      {desktop ? <View style={[styles.adminSidebar, { width: sidebarWidth }]}>
        <AdminSidebarNavigation collapsed={collapsed} expandedGroups={expandedGroups} onOpen={open} pathname={pathname} setExpandedGroups={setExpandedGroups} />
      </View> : null}
...
function AdminMobileSearch({ dark, onOpen }: { dark: boolean; onOpen(href: string): void }) {
  const [search, setSearch] = useState("");
  const [focused, setFocused] = useState(false);
  const searchResults = search.trim().length < 2 ? [] : adminLinks.filter((link) => `${link.label} ${link.href}`.toLowerCase().includes(search.trim().toLowerCase())).slice(0, 6);
  const submitSearch = () => { const match = searchResults[0]; if (match) { setSearch(""); setFocused(false); onOpen(match.href); } };
  return <View style={[styles.adminMobileSearchWrap, dark && styles.adminDarkTopbar]}>
    <View style={[styles.adminSearch, dark && styles.adminDarkSearch, focused && styles.adminSearchFocused]}><Ionicons color={colors.muted} name="search-outline" size={17} /><TextInput accessibilityLabel="Search admin pages" onBlur={() => setTimeout(() => setFocused(false), 120)} onChangeText={setSearch} onFocus={() => setFocused(true)} onSubmitEditing={submitSearch} placeholder="Search admin pages" placeholderTextColor="#94a3b8" returnKeyType="search" style={[styles.adminSearchInput, dark && styles.adminDarkText]} value={search} /></View>
    {focused && search.trim().length >= 2 ? <View style={[styles.adminMobileSearchResults, dark && styles.adminDarkResults]}>{searchResults.map((item) => <Pressable key={item.href} onPress={() => { setSearch(""); setFocused(false); onOpen(item.href); }} style={[styles.adminSearchResult, dark && styles.adminDarkResult]}><Ionicons color={colors.primary} name={item.icon as never} size={18} /><View style={styles.adminSearchCopy}><Text style={[styles.adminSearchLabel, dark && styles.adminDarkText]}>{item.label}</Text><Text style={styles.adminSearchPath}>{item.href}</Text></View></Pressable>)}{!searchResults.length ? <View style={styles.adminSearchEmpty}><Ionicons color={colors.muted} name="search-outline" size={18} /><Text style={styles.adminSearchEmptyText}>No admin page matches “{search.trim()}”.</Text></View> : null}</View> : null}
  </View>;
}

function AdminSidebarNavigation({ collapsed, expandedGroups, onOpen, pathname, setExpandedGroups }: { collapsed: boolean; expandedGroups: Record<string, boolean>; onOpen: (href: string) => void; pathname: string; setExpandedGroups: Dispatch<SetStateAction<Record<string, boolean>>> }) {
  return <>
    ...
    <ScrollView contentContainerStyle={styles.adminSidebarScroll} showsVerticalScrollIndicator={false} style={styles.adminSidebarScroller}>
      <View style={styles.adminPrimary}>{adminPrimaryLinks.map((link) => <AdminNavLink key={link.href} link={link} pathname={pathname} collapsed={collapsed} onOpen={onOpen} />)}</View>
      {adminGroups.map((group) => {
        ...
        return <View key={group.label} style={styles.adminGroup}>
          {!collapsed ? <Pressable accessibilityRole="button" onPress={() => setExpandedGroups((current) => ({ ...current, [group.label]: !expanded }))} style={[styles.adminGroupButton, groupActive && styles.adminGroupButtonActive]}><Ionicons color={groupActive ? "#ffffff" : "#cbd5e1"} name={group.icon as never} size={19} /><Text style={[styles.adminGroupButtonText, groupActive && styles.adminNavLabelActive]}>{group.label}</Text><Ionicons color="#94a3b8" name={expanded ? "chevron-up" : "chevron-down"} size={15} /></Pressable> : null}
          {collapsed || expanded ? <View style={!collapsed && styles.adminGroupChildren}>{group.links.map((link) => <AdminNavLink key={link.href} link={link} pathname={pathname} collapsed={collapsed} nested onOpen={onOpen} />)}</View> : null}
        </View>;
      })}
      <View style={styles.adminStandalone}>{adminStandaloneLinks.map((link) => <AdminNavLink key={link.href} link={link} pathname={pathname} collapsed={collapsed} onOpen={onOpen} />)}</View>
    </ScrollView>
    <Pressable onPress={() => onOpen("/")} style={[styles.storefrontLink, collapsed && styles.navCollapsed]}><Ionicons color="#cbd5e1" name="home-outline" size={19} />{!collapsed ? <Text style={styles.storefrontText}>View storefront</Text> : null}</Pressable>
  </>;
}
...
function StandardScreen({ children, description, eyebrow, hideHeading = false, pathname, title, vendor, width }: ScreenProps & { pathname: string; vendor: boolean; width: number }) {
  ...
  return <SafeAreaView style={styles.safe}>
    <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.container, { width: contentWidth }]}>{!hideHeading ? <Heading compact={width < 480} description={description} eyebrow={eyebrow} title={title} workspace={vendor} /> : null}<View style={styles.content}>{children}</View></View>
    </ScrollView>
    {showCustomerChrome && width < 900 ? <BottomNav /> : null}
  </SafeAreaView>;
}

function Heading({ compact, description, eyebrow, title, workspace }: { compact: boolean; description?: string; eyebrow?: string; title: string; workspace?: boolean }) {
  return <View style={[styles.heading, compact && styles.compactHeading, workspace && styles.workspaceHeading]}>{eyebrow ? <Text style={[styles.eyebrow, workspace && styles.workspaceEyebrow]}>{eyebrow}</Text> : null}<Text accessibilityRole="header" style={[styles.title, compact && styles.compactTitle, workspace && styles.workspaceTitle]}>{title}</Text>{description ? <Text style={[styles.description, workspace && styles.workspaceDescription]}>{description}</Text> : null}</View>;
}
...
const styles = StyleSheet.create({
  adminPrimary: { borderBottomColor: "rgba(255,255,255,.1)", borderBottomWidth: 1, gap: 3, marginBottom: 8, paddingBottom: 8 }, adminStandalone: { borderTopColor: "rgba(255,255,255,.1)", borderTopWidth: 1, marginTop: 8, paddingTop: 8 }, adminNestedActive: { backgroundColor: "rgba(255,255,255,.1)" }, childDot: { backgroundColor: "#64748b", borderRadius: radius.pill, height: 6, width: 6 }, childDotActive: { backgroundColor: "#38bdf8" },
  adminHeading: { alignItems: "flex-start", borderBottomColor: colors.border, borderBottomWidth: 1, flexDirection: "row", gap: 12, marginBottom: spacing.sm, minWidth: 0, paddingBottom: spacing.lg, paddingTop: spacing.lg }, adminBack: { alignItems: "center", backgroundColor: colors.primarySoft, borderColor: "rgba(30,112,152,.2)", borderRadius: radius.md, borderWidth: 1, flexShrink: 0, height: 40, justifyContent: "center", width: 40 }, adminHeadingCopy: { flex: 1, minWidth: 0 }, adminPill: { alignSelf: "flex-start", backgroundColor: colors.background, borderColor: colors.border, borderRadius: radius.pill, borderWidth: 1, marginBottom: 5, maxWidth: "100%", paddingHorizontal: 9, paddingVertical: 4 }, adminPillText: { color: colors.text, flexShrink: 1, fontSize: 9, fontWeight: "700", letterSpacing: .7, textTransform: "uppercase" }, adminPageTitle: { color: colors.text, flexShrink: 1, fontSize: 24, fontWeight: "700", letterSpacing: -.4 }, adminPageDescription: { color: colors.muted, flexShrink: 1, fontSize: 12, fontWeight: "600", lineHeight: 18, marginTop: 3 },
  safe: { backgroundColor: colors.background, flex: 1 }, page: { minHeight: "100%", paddingBottom: spacing.xl },
  workspaceChrome: { backgroundColor: colors.surface, borderBottomColor: colors.border, borderBottomWidth: 1 }, workspaceTop: { alignItems: "center", alignSelf: "center", flexDirection: "row", justifyContent: "space-between", minHeight: 68, paddingHorizontal: spacing.sm }, workspaceBrand: { alignItems: "center", flexDirection: "row", gap: 10 }, workspaceMark: { alignItems: "center", backgroundColor: colors.primary, borderRadius: radius.md, height: 38, justifyContent: "center", width: 38 }, adminMark: { backgroundColor: colors.navy }, workspaceMarkText: { color: colors.surface, fontSize: 15, fontWeight: "700" }, workspaceName: { color: colors.text, fontSize: 15, fontWeight: "700" }, workspaceLabel: { color: colors.muted, fontSize: 9, fontWeight: "600", letterSpacing: .7, marginTop: 1 }, workspaceActions: { alignItems: "center", flexDirection: "row", gap: spacing.sm }, iconButton: { alignItems: "center", backgroundColor: colors.background, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, height: 38, justifyContent: "center", position: "relative", width: 38 }, workspaceNav: { alignSelf: "center", gap: 5, paddingBottom: 9 }, navItem: { alignItems: "center", borderRadius: radius.md, flexDirection: "row", gap: 6, minHeight: 38, paddingHorizontal: 12 }, navItemActive: { backgroundColor: colors.primarySoft }, navLabel: { color: colors.muted, fontSize: 12, fontWeight: "600" }, navLabelActive: { color: colors.primary, fontWeight: "700" },
  adminTopbar: { alignItems: "center", backgroundColor: colors.surface, borderBottomColor: colors.border, borderBottomWidth: 1, flexDirection: "row", gap: spacing.md, height: 66, justifyContent: "space-between", paddingHorizontal: spacing.md, zIndex: 20 }, adminMobileTopbar: { gap: spacing.sm, paddingHorizontal: spacing.sm }, adminTopLeft: { alignItems: "center", flexDirection: "row", flexShrink: 1, gap: 10, minWidth: 0 }, topIcon: { alignItems: "center", borderRadius: radius.md, height: 38, justifyContent: "center", width: 38 }, adminSearchWrap: { flex: 1, maxWidth: 520, position: "relative", zIndex: 30 }, adminSearch: { alignItems: "center", backgroundColor: colors.background, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexDirection: "row", gap: 8, paddingHorizontal: 12 }, adminSearchFocused: { backgroundColor: colors.surface, borderColor: colors.primary }, adminSearchInput: { color: colors.text, flex: 1, fontSize: 13, height: 40, outlineStyle: "none" } as never, adminSearchResults: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, boxShadow: "0 12px 32px rgba(15,23,42,.18)", left: 0, overflow: "hidden", position: "absolute", right: 0, top: 46 }, adminSearchResult: { alignItems: "center", borderBottomColor: colors.border, borderBottomWidth: 1, flexDirection: "row", gap: 10, minHeight: 54, paddingHorizontal: 12, paddingVertical: 7 }, adminSearchIcon: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.sm, height: 34, justifyContent: "center", width: 34 }, adminSearchCopy: { flex: 1 }, adminSearchLabel: { color: colors.text, fontSize: 13, fontWeight: "700" }, adminSearchPath: { color: colors.muted, fontSize: 10, marginTop: 2 }, adminSearchEmpty: { alignItems: "center", flexDirection: "row", gap: 8, padding: spacing.md }, adminSearchEmptyText: { color: colors.muted, flex: 1, fontSize: 12 }, notificationDot: { backgroundColor: colors.danger, borderColor: colors.surface, borderRadius: 6, borderWidth: 2, height: 8, position: "absolute", right: 7, top: 6, width: 8 }, adminAccount: { alignItems: "center", borderLeftColor: colors.border, borderLeftWidth: 1, flexDirection: "row", gap: 9, marginLeft: 2, paddingLeft: 12 }, adminMobileAccount: { marginLeft: 0, paddingLeft: 6 }, adminMobileActions: { flexShrink: 0, gap: 5 }, logoutButton: { backgroundColor: "#fff7f7", borderColor: "#fecaca" }, avatar: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.md, height: 36, justifyContent: "center", width: 36 }, avatarText: { color: colors.primary, fontSize: 14, fontWeight: "700" }, accountName: { color: colors.text, fontSize: 12, fontWeight: "700" }, accountRole: { color: colors.muted, fontSize: 10, marginTop: 1 },
  adminMobileSearchWrap: { backgroundColor: colors.surface, borderBottomColor: colors.border, borderBottomWidth: 1, padding: spacing.sm, position: "relative", zIndex: 18 }, adminMobileSearchResults: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, left: spacing.sm, overflow: "hidden", position: "absolute", right: spacing.sm, top: 58 }, adminBody: { flex: 1, flexDirection: "row", minWidth: 0 }, adminSidebar: { backgroundColor: colors.navy, borderRightColor: "#0f1224", borderRightWidth: 1 }, adminSidebarScroller: { flex: 1 }, adminDrawerLayer: { bottom: 0, flexDirection: "row", left: 0, position: "absolute", right: 0, top: 66, zIndex: 19 }, adminDrawerBackdrop: { backgroundColor: "rgba(2,6,23,.58)", bottom: 0, left: 0, position: "absolute", right: 0, top: 0 }, adminDrawer: { backgroundColor: colors.navy, borderRightColor: "#0f1224", borderRightWidth: 1, boxShadow: "8px 0 28px rgba(2,6,23,.28)", elevation: 18, flexDirection: "column", maxWidth: 288 }, workspacePanel: { alignItems: "center", backgroundColor: "rgba(255,255,255,.08)", borderColor: "rgba(255,255,255,.1)", borderRadius: radius.md, borderWidth: 1, flexDirection: "row", gap: 9, margin: 12, padding: 11 }, workspacePanelCollapsed: { justifyContent: "center", paddingHorizontal: 4 }, workspacePulse: { backgroundColor: "#22c55e", borderRadius: 5, height: 9, width: 9 }, workspacePanelTitle: { color: colors.surface, fontSize: 12, fontWeight: "700" }, workspacePanelCopy: { color: "#94a3b8", fontSize: 10, marginTop: 2 }, adminSidebarScroll: { paddingBottom: 14, paddingHorizontal: 10 }, adminGroup: { gap: 3, marginBottom: 5 }, adminGroupButton: { alignItems: "center", borderRadius: radius.md, flexDirection: "row", gap: 10, minHeight: 42, paddingHorizontal: 11 }, adminGroupButtonActive: { backgroundColor: "rgba(30,112,152,.35)" }, adminGroupButtonText: { color: "#cbd5e1", flex: 1, fontSize: 12, fontWeight: "700" }, adminGroupChildren: { borderLeftColor: "rgba(255,255,255,.12)", borderLeftWidth: 1, marginLeft: 19, paddingLeft: 7 }, adminNavItem: { alignItems: "center", borderRadius: radius.md, flexDirection: "row", gap: 10, minHeight: 39, paddingHorizontal: 11 }, adminNavItemActive: { backgroundColor: colors.primary }, adminNavLabel: { color: "#cbd5e1", flex: 1, fontSize: 11, fontWeight: "700" }, adminNavLabelActive: { color: colors.surface, fontWeight: "700" }, activePip: { backgroundColor: colors.surface, borderRadius: 3, height: 5, width: 5 }, navCollapsed: { justifyContent: "center", paddingHorizontal: 0 }, storefrontLink: { alignItems: "center", borderTopColor: "rgba(255,255,255,.1)", borderTopWidth: 1, flexDirection: "row", gap: 10, minHeight: 54, paddingHorizontal: 21 }, storefrontText: { color: "#cbd5e1", fontSize: 12, fontWeight: "600" },
  adminMain: { flex: 1, minWidth: 0 }, adminPage: { alignSelf: "center", maxWidth: 1440, minWidth: 0, paddingBottom: spacing.xl, paddingHorizontal: spacing.lg, width: "100%" }, adminMobilePage: { paddingHorizontal: spacing.sm }, adminBreadcrumb: { alignItems: "center", flexDirection: "row", gap: 4, minHeight: 42 }, breadcrumbMuted: { color: colors.muted, fontSize: 11, fontWeight: "700" }, breadcrumbCurrent: { color: colors.text, fontSize: 11, fontWeight: "700" },
  ...
  container: { alignSelf: "center", gap: spacing.lg, paddingHorizontal: spacing.sm, paddingTop: spacing.lg }, heading: { backgroundColor: colors.navy, borderRadius: radius.xl, padding: spacing.xl }, compactHeading: { borderRadius: radius.lg, padding: spacing.lg }, workspaceHeading: { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }, eyebrow: { color: "#fdba74", fontSize: 11, fontWeight: "700", letterSpacing: 1, textTransform: "uppercase" }, workspaceEyebrow: { color: colors.accent }, title: { color: colors.surface, fontSize: 32, fontWeight: "700", letterSpacing: -0.8, lineHeight: 38 }, compactTitle: { fontSize: 26, lineHeight: 32 }, workspaceTitle: { color: colors.text }, description: { color: "#cbd5e1", fontSize: 14, lineHeight: 21, marginTop: spacing.sm }, workspaceDescription: { color: colors.muted }, content: { gap: spacing.md, minWidth: 0 }
});
