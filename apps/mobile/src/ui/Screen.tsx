...
function AdminScreen({ children, description, desktop, eyebrow, hideHeading = false, pathname, title, width }: ScreenProps & { desktop: boolean; pathname: string; width: number }) {
  ...
  return <SafeAreaView style={[styles.safe, dark && styles.adminDarkSafe]}>
    ...
    {!desktop ? <ScrollView contentContainerStyle={styles.adminMobileNav} horizontal showsHorizontalScrollIndicator={false}>{adminLinks.map((link) => <AdminNavLink key={link.href} link={link} pathname={pathname} collapsed={false} onOpen={open} />)}</ScrollView> : null}
