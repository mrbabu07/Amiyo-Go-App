    ...
    <Pressable style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.priceRow}>
          ...
          {product.originalPrice ? <Text style={styles.original}>{money(product.originalPrice)}</Text> : null}
        </View>
        <View style={styles.metaRow}>
          <Ionicons color="#f59e0b" name="star" size={13} />
          <Text style={styles.rating}>{product.rating}</Text><Text style={styles.sold}>{product.sold} sold</Text>
          <View style={styles.cart}><Ionicons color={colors.surface} name="cart-outline" size={16} /></View>
        </View>
      </View>
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  imageWrap: { aspectRatio: 1, backgroundColor: "#f1f5f9", position: "relative", width: "100%" },
  image: { height: "100%", width: "100%" },
  discount: { backgroundColor: colors.danger, borderRadius: 4, left: 7, paddingHorizontal: 6, paddingVertical: 4, position: "absolute", top: 7 },
  ...
  badge: { alignSelf: "flex-start", backgroundColor: colors.primarySoft, borderRadius: 3, color: colors.primary, fontSize: 8, fontWeight: "900", marginBottom: 6, paddingHorizontal: 5, paddingVertical: 3, textTransform: "uppercase" },
  title: { color: colors.text, fontSize: 13, fontWeight: "800", lineHeight: 18, minHeight: 36 },
  shop: { color: colors.muted, fontSize: 10, marginTop: 4 },
  ...
  metaRow: { alignItems: "center", flexDirection: "row", marginTop: 8 },
  rating: { color: colors.text, fontSize: 10, fontWeight: "700", marginLeft: 3 },
  sold: { color: colors.muted, fontSize: 9, marginLeft: 6 },
  cart: { alignItems: "center", backgroundColor: colors.primary, borderRadius: radius.sm, height: 29, justifyContent: "center", marginLeft: "auto", width: 31 }
});
