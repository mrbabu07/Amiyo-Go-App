...
const emptyForm = (): Form => ({ scope: "GLOBAL", vendorId: "", shopId: "", categoryId: "", productId: "", percentage: "6", fixed: "0", starts: today(), ends: "" });
...
const commissionPresets: Array<{ label: string; scope: Scope; percentage: string; fixed: string; copy: string }> = [
  { label: "Marketplace standard", scope: "GLOBAL", percentage: "6", fixed: "0", copy: "Default fallback for every seller." },
  { label: "New seller intro", scope: "SHOP", percentage: "3", fixed: "0", copy: "Low shop-wise rate for onboarding." },
  { label: "Category standard", scope: "CATEGORY", percentage: "8", fixed: "0", copy: "Higher-margin category rule." },
  { label: "Product boost", scope: "PRODUCT", percentage: "2", fixed: "0", copy: "Special product-wise override." }
];
...
export function AdminCommissionScreen() {
  ...
  function applyPreset(preset: typeof commissionPresets[number]) { setForm({ ...form, scope: preset.scope, percentage: preset.percentage, fixed: preset.fixed, vendorId: ["VENDOR", "VENDOR_CATEGORY"].includes(preset.scope) ? form.vendorId : "", shopId: preset.scope === "SHOP" ? form.shopId : "", categoryId: ["CATEGORY", "VENDOR_CATEGORY"].includes(preset.scope) ? form.categoryId : "", productId: preset.scope === "PRODUCT" ? form.productId : "" }); }
  ...
  return <Screen eyebrow="FINANCE CONTROL" title="Commission Rules" description="Set global, vendor, shop and product-wise commission rates.">
    ...
    <View style={styles.presetRail}>{commissionPresets.map((preset) => <Pressable key={preset.label} onPress={() => applyPreset(preset)} style={styles.preset}><Text style={styles.presetRate}>{preset.percentage}%</Text><View style={styles.ruleCopy}><Text style={styles.presetTitle}>{preset.label}</Text><Text style={styles.scopeCopy}>{preset.copy}</Text></View><Ionicons color={colors.primary} name="arrow-forward-circle-outline" size={20} /></Pressable>)}</View>
...
const styles = StyleSheet.create({
  metrics: { flexDirection: "row", flexWrap: "wrap", gap: spacing.md }, presetRail: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, preset: { alignItems: "center", backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg, borderWidth: 1, flex: 1, flexDirection: "row", gap: spacing.sm, minWidth: 220, padding: spacing.md }, presetRate: { color: colors.accent, fontSize: 20, fontWeight: "700" }, presetTitle: { color: colors.text, fontSize: 12, fontWeight: "700" }, metric: { alignItems: "center", backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg, borderWidth: 1, flex: 1, flexDirection: "row", gap: spacing.sm, minWidth: 150, padding: spacing.md }, metricIcon: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.md, height: 42, justifyContent: "center", width: 42 }, metricValue: { color: colors.text, fontSize: 21, fontWeight: "700" }, metricLabel: { color: colors.muted, fontSize: 10 },
  ...
  eyebrow: { color: colors.primary, fontSize: 9, fontWeight: "700", letterSpacing: 1 }, cardTitle: { color: colors.text, fontSize: 19, fontWeight: "700", marginTop: 3 }, close: { alignItems: "center", backgroundColor: colors.background, borderRadius: radius.pill, height: 38, justifyContent: "center", width: 38 },
  scopes: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, scope: { borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexGrow: 1, minWidth: 136, padding: spacing.sm }, scopeActive: { backgroundColor: colors.primary, borderColor: colors.primary }, scopeLabel: { color: colors.text, fontSize: 12, fontWeight: "700", marginTop: 4 }, scopeLabelActive: { color: "#fff" }, scopeCopy: { color: colors.muted, fontSize: 9, marginTop: 3 }, scopeCopyActive: { color: "#dbeafe" },
  fields: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, field: { flex: 1, gap: 6, minWidth: 190 }, label: { color: colors.text, fontSize: 11, fontWeight: "700" }, inputWrap: { alignItems: "center", borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexDirection: "row", minHeight: 46, paddingHorizontal: 12 }, input: { color: colors.text, flex: 1, minWidth: 0, outlineStyle: "none", paddingVertical: 10 } as never, affix: { color: colors.muted, fontSize: 11, fontWeight: "700", marginRight: 5 },
  ...
  primary: { alignItems: "center", backgroundColor: colors.primary, borderRadius: radius.md, flexDirection: "row", gap: 7, justifyContent: "center", minHeight: 48 }, primaryText: { color: "#fff", fontWeight: "700" }, disabled: { opacity: .5 }, error: { backgroundColor: "#fef2f2", borderRadius: radius.md, color: colors.danger, padding: spacing.sm }, refresh: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.md, height: 40, justifyContent: "center", width: 40 },
  rules: { gap: spacing.sm }, rule: { backgroundColor: colors.background, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, gap: spacing.sm, padding: spacing.md }, ruleTop: { alignItems: "flex-start", flexDirection: "row", gap: spacing.sm, justifyContent: "space-between" }, ruleIdentity: { alignItems: "center", flex: 1, flexDirection: "row", gap: spacing.sm }, ruleIcon: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.md, height: 40, justifyContent: "center", width: 40 }, ruleCopy: { flex: 1, minWidth: 0 }, ruleTitle: { color: colors.text, fontWeight: "700" }, ruleMeta: { color: colors.muted, fontSize: 9, marginTop: 2 }, status: { backgroundColor: "#fff7ed", borderRadius: radius.pill, color: colors.warning, fontSize: 8, fontWeight: "700", overflow: "hidden", paddingHorizontal: 8, paddingVertical: 5 }, active: { backgroundColor: "#ecfdf5", color: colors.success }, ended: { backgroundColor: "#f1f5f9", color: colors.muted },
  feeRow: { alignItems: "baseline", flexDirection: "row", gap: spacing.sm }, fee: { color: colors.accent, fontSize: 24, fontWeight: "700" }, fixed: { color: colors.text, fontWeight: "700" }, period: { color: colors.muted, fontSize: 10 }, actions: { borderTopColor: colors.border, borderTopWidth: 1, flexDirection: "row", gap: spacing.sm, paddingTop: spacing.sm }, action: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radius.sm, flexDirection: "row", gap: 5, paddingHorizontal: 10, paddingVertical: 8 }, actionText: { color: colors.primary, fontSize: 10, fontWeight: "700" }, endAction: { backgroundColor: "#fef2f2" }, endText: { color: colors.danger, fontSize: 10, fontWeight: "700" },
  empty: { alignItems: "center", gap: spacing.sm, padding: spacing.xl }, emptyTitle: { color: colors.text, fontWeight: "700" }, muted: { color: colors.muted }, backdrop: { alignItems: "center", backgroundColor: "rgba(15,23,42,.55)", flex: 1, justifyContent: "center", padding: spacing.md }, modal: { backgroundColor: colors.surface, borderRadius: radius.lg, maxHeight: "80%", maxWidth: 560, padding: spacing.lg, width: "100%" }, modalHeading: { alignItems: "center", flexDirection: "row", justifyContent: "space-between" }, modalSearch: { borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, color: colors.text, marginVertical: spacing.md, minHeight: 46, paddingHorizontal: 12 }, option: { alignItems: "center", borderBottomColor: colors.border, borderBottomWidth: 1, flexDirection: "row", minHeight: 54, paddingHorizontal: spacing.sm }, optionActive: { backgroundColor: colors.primarySoft }, optionLabel: { color: colors.text, fontWeight: "600" }
});
