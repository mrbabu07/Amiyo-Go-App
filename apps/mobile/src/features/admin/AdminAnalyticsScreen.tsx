...
export function AdminAnalyticsScreen() {
  ...
  return <Screen eyebrow="MARKETPLACE INTELLIGENCE" title="Analytics & customer insights" description="Live performance, purchasing segments and seller/product rankings from marketplace transactions.">
    <View style={styles.filters}>{ranges.map((item) => <Pressable accessibilityRole="button" key={item} onPress={() => setRange(item)} style={[styles.filter, range === item && styles.filterActive]}><Text style={[styles.filterText, range === item && styles.filterTextActive]}>{item.replace("d", " days")}</Text></Pressable>)}</View>
...
const styles = StyleSheet.create({ filters: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, filter: { borderColor: colors.border, borderRadius: radius.pill, borderWidth: 1, paddingHorizontal: spacing.md, paddingVertical: spacing.sm }, filterActive: { backgroundColor: colors.primary, borderColor: colors.primary }, filterText: { color: colors.muted, fontWeight: "800" }, filterTextActive: { color: colors.surface }, metrics: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, metric: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg, borderWidth: 1, flexBasis: 160, flexGrow: 1, gap: 4, padding: spacing.md }, metricLabel: { color: colors.muted, fontSize: 12, fontWeight: "800" }, metricValue: { color: colors.text, fontSize: 22, fontWeight: "900" }, heading: { color: colors.text, fontSize: 21, fontWeight: "900" }, trendRow: { alignItems: "center", flexDirection: "row", gap: spacing.sm }, date: { color: colors.muted, fontSize: 11, width: 52 }, track: { backgroundColor: colors.primarySoft, borderRadius: radius.pill, flex: 1, height: 9, overflow: "hidden" }, bar: { backgroundColor: colors.primary, borderRadius: radius.pill, height: 9 }, trendValue: { color: colors.text, fontSize: 11, fontWeight: "700", textAlign: "right", width: 78 }, rankingValue: { color: colors.primary, fontSize: 20, fontWeight: "900" }, muted: { color: colors.muted, fontSize: 12 }, error: { color: colors.danger } });
