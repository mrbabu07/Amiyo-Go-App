...
import { z } from "zod";
import { answerInputSchema, chatMessageInputSchema, chatThreadInputSchema, contentModerationInputSchema, createPromotionSchema, questionInputSchema, reviewInputSchema, stockAlertInputSchema, wishlistItemInputSchema } from "@amiyo/contracts";
...
export function createEngagementRouter() {
  ...
  router.use(["/api/v2/wishlist", "/api/v2/alerts", "/api/v2/reviews", "/api/v2/questions", "/api/v2/notifications", "/api/v2/chat", "/api/v2/loyalty", "/api/v2/vendor/engagement", "/api/v2/admin/content", "/api/v2/admin/promotions"], authenticate);
  ...
  router.post("/api/v2/catalog/products/:productId/questions", writeLimit, async (req, res, next) => { try { res.status(201).json(await service.createQuestion(requireSession(req), productSchema.parse(req.params).productId, questionInputSchema.parse(req.body))); } catch (error) { next(error); } });
