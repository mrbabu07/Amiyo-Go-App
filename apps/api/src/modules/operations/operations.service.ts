...
import { assertAvailableBalance, assertRefundLimit, calculateLedgerBalance, canTransitionReturn, minimumPayoutMinor, providerReferenceValid, refundProviderRule, settlementDecision } from "@amiyo/domain";
...
export class OperationsService {
  ...
  async requestPayout(session: Session, input: CreatePayoutRequest, key: string, correlationId?: string) {
    ...
    return withSerializableTransaction(this.client, async (transaction) => {
      ...
      const current = await wallet(transaction, vendorId); const amount = BigInt(input.amountMinor); if (amount < minimumPayoutMinor) throw new ApiProblem(409, "PAYOUT_BELOW_MINIMUM", "Payout amount is below the BDT 500 minimum settlement threshold"); try { assertAvailableBalance(calculateLedgerBalance(current.entries), amount); } catch { throw new ApiProblem(409, "PAYOUT_BALANCE_INSUFFICIENT", "Payout exceeds available balance"); }
  ...
  async completePayout(session: Session, id: string, input: CompletePayout, key: string) {
    ...
    return withSerializableTransaction(this.client, async (transaction) => {
      const prior = await replay(transaction, scope, key, input); if (prior.response) return prior.response; const current = await transaction.vendorPayoutRequest.findUnique({ where: { id }, include: { bankAccount: { select: { verifiedAt: true } } } }); if (!current) throw new ApiProblem(404, "PAYOUT_NOT_FOUND", "Payout request not found"); const settlement = settlementDecision({ amountMinor: current.amountMinor, status: current.status, provider: input.provider, providerRef: input.providerRef, bankAccountVerified: Boolean(current.bankAccount.verifiedAt) }); if (!settlement.eligible) throw new ApiProblem(409, settlement.code, settlement.reason);
