...
test("coupon updates reject stale versions", async () => { const client = { coupon: { findUnique: async () => ({ id: "coupon-1", code: "SAVE10", version: 3, active: true }), updateMany: async () => ({ count: 0 }) } } as unknown as PrismaClient; await assert.rejects(() => new AdminService(client).updateCoupon(admin, "coupon-1", { code: "SAVE20", discountType: "PERCENT", value: 20, maxDiscountMinor: null, minimumSpendMinor: "0", usageLimit: null, perUserLimit: 1, startsAt: "2026-08-15T00:00:00.000Z", endsAt: "2026-08-30T00:00:00.000Z", active: true, expectedVersion: 2 }), /Coupon changed; refresh and try again/); });
test("redeemed coupons cannot be deleted", async () => { const client = { coupon: { findUnique: async () => ({ id: "coupon-1", code: "USED10", version: 1, _count: { redemptions: 2 } }) } } as unknown as PrismaClient; await assert.rejects(() => new AdminService(client).deleteCoupon(admin, "coupon-1"), /Redeemed coupons cannot be deleted/); });
