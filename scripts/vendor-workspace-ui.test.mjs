...
test("seller product workspace supports create, edit, detail and moderation submission", async () => {
  ...
  for (const capability of ["createVendorProduct", "updateVendorProduct", "submitVendorProduct", "replaceVendorProductVariants", "replaceVendorProductMedia", "pickAndUploadImage", "ProductCategorySelect", "Search seller products", "Variant generator"]) assert.match(sources, new RegExp(capability));
});
...
test("seller product media and variant APIs enforce transactional replacement", async () => {
  ...
  for (const endpoint of ["/:id/variants", "/:id/media"]) assert.match(routes, new RegExp(endpoint.replaceAll("/", "\\/")));
  for (const guard of ["VERSION_CONFLICT", "MEDIA_UPLOAD_NOT_READY", "VARIANT_NOT_OWNED", "STOCK_BELOW_RESERVED", "catalog.product_media.replaced", "catalog.product_variants.replaced"]) assert.match(service, new RegExp(guard));
});
