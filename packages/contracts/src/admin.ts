...
export const platformFeatureFlagsSchema = z.object({ sellerRegistration: z.boolean(), reviews: z.boolean(), questions: z.boolean(), flashSales: z.boolean(), loyalty: z.boolean(), voiceSearch: z.boolean(), googleLogin: z.boolean() });
export const platformPaymentMethodsSchema = z.object({ cod: z.object({ enabled: z.boolean() }), sslcommerz: z.object({ enabled: z.boolean() }), bkashManual: z.object({ enabled: z.boolean() }) });
export const platformSettingsInputSchema = z.object({ expectedVersion: versionSchema, featureFlags: platformFeatureFlagsSchema, paymentMethods: platformPaymentMethodsSchema, maintenanceMode: z.object({ enabled: z.boolean(), message: z.string().trim().min(10).max(500) }), returnPolicy: z.object({ windowDays: z.number().int().min(1).max(90), restockingFeePercent: z.number().min(0).max(100), requireEvidence: z.boolean() }), tax: z.object({ enabled: z.boolean(), rateBps: z.number().int().min(0).max(10000), pricesIncludeTax: z.boolean() }), seo: z.object({ title: z.string().trim().min(3).max(120), description: z.string().trim().min(10).max(300) }), notificationControls: z.object({ orderUpdates: z.boolean(), promotions: z.boolean(), sellerAlerts: z.boolean(), adminAlerts: z.boolean() }) });
export const platformSettingsSchema = platformSettingsInputSchema.omit({ expectedVersion: true }).extend({ key: z.literal("default"), version: versionSchema, updatedBy: uuidSchema.nullable(), updatedAt: timestampSchema, health: z.object({ overall: z.enum(["healthy", "degraded"]), checkedAt: timestampSchema, environment: z.string(), services: z.array(z.object({ key: z.string(), label: z.string(), status: z.enum(["healthy", "degraded"]), detail: z.string() })) }) });
const adminCouponFieldsSchema = z.object({ code: z.string().trim().min(3).max(40).transform((value) => value.toUpperCase()), discountType: z.enum(["PERCENT", "FIXED"]), value: z.number().int().positive(), maxDiscountMinor: z.string().regex(/^\d+$/).nullable().default(null), minimumSpendMinor: z.string().regex(/^\d+$/).default("0"), usageLimit: z.number().int().positive().nullable().default(null), perUserLimit: z.number().int().positive().default(1), startsAt: timestampSchema, endsAt: timestampSchema, active: z.boolean().default(true) });
function validateAdminCoupon(value: z.infer<typeof adminCouponFieldsSchema>, context: z.RefinementCtx) { if (new Date(value.startsAt) >= new Date(value.endsAt)) context.addIssue({ code: "custom", message: "startsAt must precede endsAt", path: ["endsAt"] }); if (value.discountType === "PERCENT" && value.value > 100) context.addIssue({ code: "custom", message: "Percentage discount cannot exceed 100", path: ["value"] }); if (value.usageLimit !== null && value.perUserLimit > value.usageLimit) context.addIssue({ code: "custom", message: "Per-user limit cannot exceed the total usage limit", path: ["perUserLimit"] }); }
export const adminCouponInputSchema = adminCouponFieldsSchema.superRefine(validateAdminCoupon);
export const updateAdminCouponSchema = adminCouponFieldsSchema.extend({ expectedVersion: versionSchema }).superRefine(validateAdminCoupon);
export const adminCouponSchema = z.object({ id: uuidSchema, code: z.string(), discountType: z.enum(["PERCENT", "FIXED"]), value: z.number().int().positive(), maxDiscountMinor: z.string().regex(/^\d+$/).nullable(), minimumSpendMinor: z.string().regex(/^\d+$/), currency: z.string().length(3), usageLimit: z.number().int().positive().nullable(), perUserLimit: z.number().int().positive(), redemptions: z.number().int().nonnegative(), redeemedMinor: z.string().regex(/^\d+$/), startsAt: timestampSchema, endsAt: timestampSchema, active: z.boolean(), status: z.enum(["active", "scheduled", "expired", "disabled"]), version: versionSchema });
const adminCampaignFieldsSchema = z.object({ name: z.string().trim().min(3).max(120), slug: z.string().trim().min(3).max(120).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), description: z.string().trim().min(10).max(2000), discountType: z.enum(["PERCENT", "FIXED"]), discountValue: z.number().positive(), imageUrl: z.string().url().nullable().default(null), startsAt: timestampSchema, endsAt: timestampSchema, status: z.enum(["draft", "active", "paused"]).default("draft"), showAsPopup: z.boolean().default(true), priority: z.number().int().nonnegative().max(10000).default(0), couponCode: z.string().trim().regex(/^[A-Z0-9_-]{3,40}$/).nullable().default(null), productIds: z.array(uuidSchema).max(250).default([]), buttonText: z.string().trim().min(2).max(40).default("Shop now"), buttonLink: z.string().trim().min(1).max(300).refine((value) => value.startsWith("/"), "Button link must be an internal route") });
function validateAdminCampaign(value: z.infer<typeof adminCampaignFieldsSchema>, context: z.RefinementCtx) { if (new Date(value.startsAt) >= new Date(value.endsAt)) context.addIssue({ code: "custom", message: "startsAt must precede endsAt", path: ["endsAt"] }); if (value.discountType === "PERCENT" && value.discountValue > 100) context.addIssue({ code: "custom", message: "Percentage discount cannot exceed 100", path: ["discountValue"] }); if (new Set(value.productIds).size !== value.productIds.length) context.addIssue({ code: "custom", message: "Campaign products must be unique", path: ["productIds"] }); }
export const adminCampaignInputSchema = adminCampaignFieldsSchema.superRefine(validateAdminCampaign);
...
export const adminInventoryMovementSchema = z.object({ id: uuidSchema, inventoryId: uuidSchema, type: z.enum(["RECEIPT", "ADJUSTMENT", "RESERVATION", "RELEASE", "SALE", "RETURN"]), quantity: z.number().int(), referenceType: z.string(), referenceId: z.string(), reason: z.string().nullable(), createdAt: timestampSchema });
export const adminInventoryItemSchema = z.object({ id: uuidSchema, version: versionSchema, productId: uuidSchema, product: z.string(), productStatus: z.string(), sku: z.string(), variant: z.string(), vendor: z.string(), shop: z.string(), priceMinor: z.string().regex(/^\d+$/), currency: z.string().length(3), onHand: z.number().int().nonnegative(), reserved: z.number().int().nonnegative(), available: z.number().int().nonnegative(), reorderLevel: z.number().int().nonnegative(), stockValueMinor: z.string().regex(/^\d+$/), status: z.enum(["in_stock", "low_stock", "out_of_stock"]), updatedAt: timestampSchema, movements: z.array(adminInventoryMovementSchema) });
...
export type UpdateAdminCoupon = z.infer<typeof updateAdminCouponSchema>;
export type AdminCouponDto = z.infer<typeof adminCouponSchema>;
export type AdminCampaignInput = z.infer<typeof adminCampaignInputSchema>;
